 ...
 */
        ...
        {
            /* Valles methode hat bei 1000000 iterationen * 8 verschiedene bbs * 64 felder 18.99s gebraucht
            dh für 512 000 000 funktionsausführungen

            chat gpts version: halb implemented: 31.16s
            chats gpts version: voll implemented: 46.8

            valles verision mit ternary operators anstatt if-statements: 
            gleich schnell wie mit if statements

            valles version, komplett finished getPseudoTargetSqrsRook(...) braucht:
            26.08s

            Performance von getPseudoLegalMoves_bb( ) mit dem Rook piece und 4 testboards 
            400 000 000 iterationen: 15.07s
            40 000 000 iterationen: 2.7s

            mit mehr caching:
            25.98s
            26.16s

            mit modulo optimierung:
            27.26s (wahrscheinlich weil compiler es besser optimiert als ich)

            mit precheck ob rook am rand ist:
            23.96s
            23.97s

            mit anti caching am ende (maybe kann der compiler hier was raussqueezen):
            zuerst horizontal: 24.4s (nope)
            zuerst vertikal: 24.08s (lohnt sich nicht)

            -->40mio iterationen: 2.41s

            mit bitshifts anstatt multiplikation (wieder auf 400mio):
            24.06s (lohnt sich nicht)
            ...
            vergleich zur getPseudoLegalMoves( ) funktion ohne Bitboards:
            40 000 000 Iterationen: 25.05s
             */
            ...
            sw.Start();
            var cb = new Chessboard(Utility.FENtoPosition("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR"));
            DisplayBoard(cb);
            Console.WriteLine(Minimax.miniMax(cb, 6, float.MinValue, float.MaxValue, true));
            Console.WriteLine("Positions: " + Minimax.positions);
